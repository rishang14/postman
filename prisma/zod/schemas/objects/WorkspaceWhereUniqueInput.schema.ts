...
import { WorkspaceOwnerIdNameCompoundUniqueInputObjectSchema as WorkspaceOwnerIdNameCompoundUniqueInputObjectSchema } from './WorkspaceOwnerIdNameCompoundUniqueInput.schema'

const makeSchema = () => z.object({
  id: z.string().optional(),
  ownerId_name: z.lazy(() => WorkspaceOwnerIdNameCompoundUniqueInputObjectSchema).optional()
}).strict().superRefine((obj, ctx) => {
        const presentTop = (k: string) => (obj as any)[k] != null;
        const singles: string[] = ["id"] as string[];
        const groups: string[][] = [["ownerId","name"]] as string[][];
