...
import { WorkspaceMemberWorkspaceIdUserIdCompoundUniqueInputObjectSchema as WorkspaceMemberWorkspaceIdUserIdCompoundUniqueInputObjectSchema } from './WorkspaceMemberWorkspaceIdUserIdCompoundUniqueInput.schema'

const makeSchema = () => z.object({
  id: z.string().optional(),
  workspaceId_userId: z.lazy(() => WorkspaceMemberWorkspaceIdUserIdCompoundUniqueInputObjectSchema).optional()
}).strict().superRefine((obj, ctx) => {
        const presentTop = (k: string) => (obj as any)[k] != null;
        const singles: string[] = ["id"] as string[];
        const groups: string[][] = [["workspaceId","userId"]] as string[][];
